...
type EvidenceAction = "LINK" | "UNLINK";
...
function SupportFormGoalsContent() {
  ...
  const [pendingEvidenceAction, setPendingEvidenceAction] = useState<{ goalId: string; entryId: string; action: EvidenceAction } | null>(null);
  ...
  async function toggleProgress(goalId: string) {
    if (expandedGoalId === goalId) {
      setExpandedGoalId(null);
      return;
    }
    await viewProgress(goalId);
  ...
  function openAction(goalId: string, action: GoalAction) {
    ...
    if (action === "EVIDENCE") { setEvidenceMessage(null); void viewProgress(goalId); }
  }
  ...
  async function linkEntry(goalId: string, entryId: string) {
    if (!formId || pendingEvidenceAction) return;
    const entry = form?.entries.find((candidate) => candidate.id === entryId);
    setError(null); setEvidenceMessage(null); setPendingEvidenceAction({ goalId, entryId, action: "LINK" });
    try {
      await api.post(`/support-forms/${formId}/goals/${goalId}/link-entry`, { supportFormEntryId: entryId });
      const updated = await api.get<Progress>(`/support-forms/${formId}/goals/${goalId}/progress`);
      setProgress(updated);
      setGoals((current) => current.map((goal) => goal.id === goalId ? { ...goal, _count: { ...goal._count, linkedEntries: updated.linkedEntries.length } } : goal));
      setEvidenceMessage(`Linked accomplishment to this goal${entry ? `: ${entry.rawText}` : "."}`);
    } catch (requestError) { setError(messageFor(requestError)); setEvidenceMessage("The accomplishment was not linked. Try again or confirm you are the rated Soldier or assigned rater."); }
    finally { setPendingEvidenceAction(null); }
  }

  async function unlinkEntry(goalId: string, entryId: string) {
    if (!formId || pendingEvidenceAction) return;
    setError(null); setEvidenceMessage(null); setPendingEvidenceAction({ goalId, entryId, action: "UNLINK" });
    try {
      await api.delete(`/support-forms/${formId}/goals/${goalId}/link-entry/${entryId}`);
      const updated = await api.get<Progress>(`/support-forms/${formId}/goals/${goalId}/progress`);
      setProgress(updated);
      setGoals((current) => current.map((goal) => goal.id === goalId ? { ...goal, _count: { ...goal._count, linkedEntries: updated.linkedEntries.length } } : goal));
      setEvidenceMessage("Removed the accomplishment from this goal.");
    } catch (requestError) { setError(messageFor(requestError)); setEvidenceMessage("The accomplishment was not unlinked. Try again or confirm your access."); }
    finally { setPendingEvidenceAction(null); }
  }
  ...
  return <div className="max-w-4xl space-y-5 p-6">
    <PageHeader icon={Target} eyebrow="Intent and traceability" title="Goals and Progress" description="Soldier-authored goals establish intent; linked accomplishments provide evidence; Soldier and rater assessments remain separate." tone="people" actions={formId && <Button variant="outline" asChild><Link href={`/support-form?formId=${formId}`}>Back to Support Form</Link></Button>} />
...
function GoalProgressDetails({ progress }: { progress: Progress }) {
  const latestPercent = [...progress.progressTrend].reverse().find((point) => point.percentAchieved !== null)?.percentAchieved ?? null;
  return <section className="mt-4 border-t border-border pt-4" aria-label={`${progress.title} progress`}><div className="grid gap-4 md:grid-cols-2"><div><p className="text-xs font-semibold uppercase text-muted-foreground">Assessment</p><p className="mt-1 text-sm">Soldier: {progress.soldierAssessment?.replaceAll("_", " ") ?? "not assessed"}</p><p className="text-sm">Rater: {progress.raterAssessment?.replaceAll("_", " ") ?? "not assessed"}</p></div><div><p className="text-xs font-semibold uppercase text-muted-foreground">Counseling progress</p>{latestPercent === null ? <p className="mt-1 text-sm text-muted-foreground">No rater percentage recorded yet.</p> : <><div className="mt-2 h-2 overflow-hidden rounded-sm bg-muted" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={latestPercent}><div className="h-full bg-primary" style={{ width: `${latestPercent}%` }} /></div><p className="mt-1 text-xs text-muted-foreground">Latest rater judgment: {latestPercent}%</p></>}<ProgressSparkline values={progress.progressTrend.map((point) => point.percentAchieved)} /></div></div><div className="mt-4 space-y-2"><p className="text-xs font-semibold uppercase text-muted-foreground">Linked accomplishments</p>{progress.linkedEntries.length ? progress.linkedEntries.map((link) => <div key={link.supportFormEntry.id} className="border-l-2 border-primary/40 pl-3 text-sm">{new Date(link.supportFormEntry.entryDate).toLocaleDateString()} - {link.supportFormEntry.rawText}</div>) : <p className="text-sm text-muted-foreground">No accomplishments linked yet.</p>}</div></section>;
}
...
function ProgressSparkline({ values }: { values: (number | null)[] }) {
  ...
  // The parent (GoalProgressDetails) owns the "Counseling progress" heading,
  // the empty state, and the latest-percentage readout. This renders only the
  // trend line itself, and only when there are at least two real data points —
  // a single point is not a trend and a zero-point chart is just noise.
  if (points.length < 2) return null;
  const width = 180; const height = 52;
  const polyline = points.map((value, index) => `${index * width / (points.length - 1)},${height - value / 100 * height}`).join(" ");
  return <svg className="mt-3 h-14 w-48 overflow-visible" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Goal percentage trend across ${points.length} counseling sessions, currently ${points.at(-1)}%`}><line x1="0" y1={height} x2={width} y2={height} stroke="currentColor" opacity="0.2" /><polyline points={polyline} fill="none" stroke="currentColor" strokeWidth="2" /></svg>;
}
